...
local ok_lsql, path_lsql = ensure.ensure_lazysql()
assert_tool(ok_lsql, path_lsql, 'lazysql')
local ver_lsql = vim.system({ path_lsql, '--version' }, { text = true }):wait()
harness.assert_eq(ver_lsql.code, 0, 'lazysql --version')
harness.ok('lazysql installed: ' .. path_lsql)
